# 👋 Hi there, I'm David Lonski!  

![download](https://github.com/user-attachments/assets/9739ad48-f966-42ac-8400-5d758e03c52b)


Welcome to my GitHub profile! I'm a **Computer Science student** from **SUNY Brockport** (minoring in Math) passionate about leveraging technology to solve real-world problems.

My journey has led me to explore a variety of fields within computer science, with a strong focus on **Backend Development**, **Cloud Infrastructure**, and **AI Integration**.

---

## 🌟 Highlights  

### **[EDU Ally Project](https://github.com/SUNY-Brockport-ACM-Student-Chapter)**
* **Role:** Backend Lead
* **Tech Stack:** Next.js, Supabase, pgvector, Docker, GCP
* **Description** Lead the migration from a Flask backend to a scalable Next.js server architecture.

### **[EDU Ally Pilot](https://github.com/davidlonski/RAG-Dev.git)**
* **Role:** Project Developer
* **Tech Stack:** Streamlit, Supabase, ChromaDB, GCP
* **Description:** Developed an AI study assistant that utilizes Retrieval-Augmented Generation (RAG) to process and interact with educational PowerPoint files.

### **[Independent Study: Generative AI](https://github.com/davidlonski/IndependentStudy-GAN.git)**
* **Focus:** GANS, R-CNN, CNN
* **Types:** DC-GAN, W-GAN, Mask-RCNN
* **Description:** Exploring the architecture and application of GANs to generate synthetic medical images.

---

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)

**Frameworks & Libraries**
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)
![PyQt](https://img.shields.io/badge/PyQt-41CD52?style=flat&logo=qt&logoColor=white)

**Cloud & DevOps**
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=flat&logo=google-cloud&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat&logo=nginx&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

## 🌱 What I’m Learning  
- **Large Language Models:** Deep diving into the Google Gemini API and RAG implementations.
- **Vector Databases:** Managing embeddings with ChromaDB and pgvector.
- **Computer Vision:** Advanced techniques for object recognition.
- **CI/CD:** Building automated pipelines from GitHub to GCP.
  
---

## 💬 Ask Me About  
- **Artificial Intelligence:** Specifically GANs and LLM integrations.
- **Backend Architecture:** Designing scalable APIs and database management.

---

## 📫 How to Reach Me  
Feel free to connect with me via email or LinkedIn:  
- 📧 Email: **davelonski12@outlook.com**
- 💼 LinkedIn: **[www.linkedin.com/in/davidlonski](https://www.linkedin.com/in/davidlonski)**

---

## ⚡ Fun Fact  
When I’m not coding, you can find me exploring outdoors, working out, or experimenting with new recipes in the kitchen.

---

Thanks for visiting my GitHub! Let's collaborate and build something amazing together. 🚀
